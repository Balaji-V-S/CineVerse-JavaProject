...
public class CineVerseConsoleApp {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 2: login(); break;
                case 3:
                    if (loggedInUser != null) showLoggedInMenu();
    ...
    private static void showLoggedInMenu() {
        while (loggedInUser != null) {
            ...
            if (loggedInUser instanceof Admin) System.out.println("4. Add New Movie (Admin)");
            ...
            switch (choice) {
                ...
                case 4: if (loggedInUser instanceof Admin) addMovie(); else System.out.println("Invalid choice."); break;
                case 9: logout(); break;
                default: System.out.println("Invalid choice. Please try again.");
            }
        }
    ...
    private static void initializeAdmin() {
        ...
        } catch (com.cineverse.exception.AllExceptions.UserAlreadyExistsException e) {
    ...
    private static void addMovie() {
        try {
            System.out.println("\n--- Add New Movie ---");
            System.out.print("Enter movie title: "); String title = scanner.nextLine();
            System.out.print("Enter genre: "); String genre = scanner.nextLine();
            System.out.print("Enter duration (minutes): "); int duration = getIntInput(); scanner.nextLine();
            System.out.print("Enter release date (YYYY-MM-DD): "); LocalDate releaseDate = LocalDate.parse(scanner.nextLine());
            List<ShowTime> showTimes = new ArrayList<>();
            System.out.println("Enter showtimes (type 'done' when finished):");
            while(true) {
                System.out.print("Enter showtime (YYYY-MM-DD HH:mm): "); String dtStr = scanner.nextLine();
                ...
                try {
                    ...
                    System.out.print("Enter available seats for this showtime: "); int seats = getIntInput(); scanner.nextLine();
                    ShowTime st = new ShowTime();
                    st.setStartTime(startTime);
                    st.setAvailableSeats(seats);
                    showTimes.add(st);
                } catch (DateTimeParseException e) { System.out.println("Invalid date/time format. Please use YYYY-MM-DD HH:mm."); }
            }
            movieService.addMovie(loggedInUser, title, genre, duration, releaseDate, showTimes);
            System.out.println("Movie added successfully!");
        } catch (Exception e) { System.out.println("Error adding movie: " + e.getMessage()); }
    }

    private static void viewAllMovies() {
        System.out.println("\n--- Available Movies ---");
        List<Movie> movies = movieService.getAllMovies();
        if (movies.isEmpty()) { System.out.println("No movies available at the moment."); return; }
        movies.forEach(movie -> {
            ...
            movie.getShowTimes().forEach(st -> System.out.printf("    - ID: %d, Time: %s, Seats Available: %d%n",
                    st.getShowTimeId(), st.getStartTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")), st.getAvailableSeats()));
        });
    }
    ...
    private static void bookTickets() {
        try {
            System.out.println("\n--- Book Tickets ---");
            viewAllMovies();
            System.out.print("Enter the Showtime ID you want to book: "); long showTimeId = getLongInput();
            System.out.print("Enter number of seats: "); int seats = getIntInput(); scanner.nextLine();
            bookingService.bookTickets(loggedInUser.getUserId(), showTimeId, seats);
            System.out.println("Booking successful!");
        } catch (Exception e) { System.out.println("Error booking tickets: " + e.getMessage()); }
    }

    private static void addReview() {
        try {
            System.out.println("\n--- Add a Review ---");
            movieService.getAllMovies().forEach(movie -> System.out.printf("ID: %d, Title: %s%n", movie.getMovieId(), movie.getTitle()));
            System.out.print("Enter the Movie ID you want to review: "); long movieId = getLongInput();
            System.out.print("Enter rating (1-5): "); int rating = getIntInput(); scanner.nextLine();
            System.out.print("Enter your comment: "); String comment = scanner.nextLine();
            reviewService.addReview(loggedInUser.getUserId(), movieId, comment, rating);
            System.out.println("Thank you for your review!");
        } catch (Exception e) { System.out.println("Error submitting review: " + e.getMessage()); }
    }
